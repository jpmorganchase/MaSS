...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # require_reshape = self.bn and len(x.shape) == 3
        # old_shape = x.shape
        # if require_reshape:
        #     x = x.reshape((old_shape[0] * old_shape[1], old_shape[2]))

        x = self.net(x)

        # if require_reshape:
        #     x = x.reshape((old_shape[0], old_shape[1], x.shape[1]))
...
def motionsense_gen_mlp(in_chans, **kwargs):
    ...
    model = ResMLPGeneratorWUnitGaussianNoise(
        layer_dims=layer_cfg, num_block=1, with_residual=kwargs['with_residual'], noise_dim=256)
